...
- journald on the Orin is **persistent** (`/etc/systemd/journald.conf.d/10-belfry-persistent.conf`), so logs survive a reboot and an unclean restart leaves a post-mortem. It was runtime-only until 2026-08-09 — `journalctl --list-boots` reported a single synthetic boot spanning Feb 2025→now, which is why the Jul 28 and Aug 9 unclean reboots have no cause on record. `SystemMaxUse=1G` / `SystemKeepFree=10G` are pinned deliberately: `/` is the 3.6 TB recordings volume and already sits past the 85% eviction watermark, so an unbounded journal would compete with the space `dvr/retention.py` is working to reclaim.
