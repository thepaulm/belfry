...
Self-hosted DVR for Interlogix TVB-5301 cameras (OEM Hikvision) on an isolated camera subnet. The on-prem Jetson Orin pulls RTSP from each camera; MediaMTX repackages to HLS for live viewing and writes 1-hour fMP4 segments to disk for recording; a small FastAPI service serves a per-set browser viewer plus a per-camera scrubback page. nginx fronts everything on port 80 for LAN access. A small EC2 frontdoor (Caddy + oauth2-proxy) terminates HTTPS for `example.com`, gates every request behind Google OAuth, and reverse-proxies through a persistent SSH tunnel back to the Orin — so all video and recordings stay on the Orin while the public side enforces auth.
...
- `inference/` — object detection pipeline (Phase 4):
  - `model.py` — `Detector` wrapping a single YOLO11l — the **belfry-v1 fine-tune** (COCO's 80 classes + 6 appended wildlife classes deer/coyote/raccoon/rabbit/squirrel/rat) — filtering to the `person/dog/cat/bird/car/truck` + wildlife subset via `_YOLO_KEEP`, then renaming via `class_aliases` (default car/truck → `vehicle` — the distinction is noise for us, and class-flicker on one pickup used to open two parallel event runs); thread-safe so one instance is shared across all camera threads. Replaced an earlier MegaDetector-larch + YOLO11n ensemble whose IoU-merge produced too many low-confidence person false-positives; one model end-to-end means the live recorder and past-mode re-inference can't disagree.
  - `recorder.py` — `EventRecorder` per camera, two threads. A **drain thread** does a tight `cv2.VideoCapture.read()` loop on the loopback RTSP at native ~30 fps and stores only the latest frame in `self._latest`. A separate **inference loop** wakes at `record_fps` (default 1 fps), grabs `self._latest`, runs the detector, coalesces detections into event runs (open / extend / close on cooldown), writes one row per closed run to SQLite + a peak-conf JPEG thumbnail, and publishes every detection (incl. empty batches) to `live.broadcaster`. The drain/sample split exists because a single-thread version that slept ~1 s between reads let OS-level TCP buffers fill, MediaMTX's RTSP packets stack up, and the H.264 stream eventually corrupt — `read()` returned `False`, the recorder reopened, and a few seconds of frames were lost (past-mode re-inference then found events the live path missed). Drain thread keeps the socket continuously consumed; inference cadence stays decoupled.
  ...
  - `runner.py` — production multi-camera coordinator; spawns one EventRecorder thread per camera with `inference: true`, plus a uvicorn thread for the SSE server, all sharing one Detector. Also runs a **watchdog thread**: if any recorder that has produced frames goes frame-silent for >120 s (its drain thread wedged inside cv2's blocking `VideoCapture` open — the recurring failure after a MediaMTX blip 404-storms the loopback paths; the thread stays `is_alive()` so the "all threads died" check can't catch it), it `os._exit(1)`s so systemd rebuilds every capture. Bounds that outage to ~2 min instead of the ~28 h a silent wedge cost twice (2026-05-16, 2026-06-30). Never-produced-a-frame cameras are excluded so a permanently-down feed can't crash-loop the service.
...
- `scripts/import-external.py` — pulls an external YOLO-format dataset (e.g. a Roboflow trailcam set) into `images/`+`labels/`, remapping its classes to our sparse ids by name and dropping unmapped ones. `--max-per-class`/`--max-negatives` cap the import so a lopsided source can't dominate; files land with an `ext_<tag>_` prefix so `split-dataset.py` keeps them out of the our-footage val metric.
- `scripts/split-dataset.py` — deterministic hash-based 90/10 train/val split; writes `train.txt`/`val.txt` + a dense `nc=86` `dataset.train.yaml` (so sparse COCO-aligned label ids stay in range) and reports the our-footage val count separately.
- `scripts/extend-head.py` — surgically extends YOLO11l's head 80→86, copying the COCO channels verbatim so base-class predictions are unchanged; produces the `*-headext.pt` that feeds training with `freeze=10`.
- `scripts/runpod-{1-send-to-pod,2-train-on-pod,3-receive-weights,4-export-and-swap}.sh` — the four-step *manual* RunPod retrain flow (bundle+send dataset via `runpodctl` p2p → train on the pod → pull weights back → TRT-export on the Orin + flip cameras.yaml + restart). `scripts/runpod-version` holds the `VERSION=` bumped once per run; step 1 bakes it into the train-script copy it ships so the pod side can't disagree on names. Step 2 never runs from the repo — it rides inside the tarball as `train-on-pod.sh`. Prereqs per run: bump the version, re-run `split-dataset.py`. Kept for hands-on / debugging runs.
- `scripts/runpod-auto.sh` — **one-command, unattended** version of the manual 4-step flow. Validated end-to-end 2026-06-10. Flow: `runpodctl pod create` (default `NVIDIA A40` — ~$0.47/run, slower wall-clock but turnaround doesn't matter for overnight retrains, and the Blackwell PRO 6000 is frequently out of secure-cloud capacity anyway; override `BELFRY_GPU_ID` with the Blackwell string when you want it fast) → push the same dataset bundle (croc) → run `train-on-pod.sh` detached, poll its log → pull `best.pt` (croc) → **delete the pod the instant weights land** (before the Orin-side export, so no idle-GPU billing) → `runpod-4-export-and-swap.sh`. Reuses `runpod-version` + `runpod-2`/`runpod-4`. **Hard-won RunPod quirks baked in (don't "simplify" these away):** (1) `runpodctl ssh info`/`pod get` report "pod not ready"/`uptimeSeconds:0` *indefinitely* on secure-cloud pods even when SSH works — so the SSH **username** (`machine.podHostId`, an opaque `<id>-<perPodSuffix>`) and **readiness** (`runtime != null`) come from the **GraphQL API** (`api.runpod.io/graphql`, key from config.toml), not runpodctl. The suffix is per-pod (a counter, not a key fingerprint) — never hardcode it. (2) RunPod's proxy SSH (`ssh.runpod.io`) drops into an *interactive* shell that ignores a command arg and has no scp — so `pod_ssh()` feeds commands on **stdin**, silences prompt/echo (`PS1=''`/`stty -echo`), brackets output with markers, strips ANSI, and recovers the remote exit code from the marker; bulk transfer stays on **croc**. (3) RunPod images ship an ancient `runpodctl` (1.14.x) whose croc protocol is incompatible — the script **pins the pod's runpodctl to match** (`BELFRY_RUNPODCTL_VER`). (4) croc rewrites a passed `--code` (appends a shard count) — so **don't pass `--code`; parse the real code** from `send`'s output. (5) every fragile `var=$(…|grep…)` needs `|| true` or `set -e`+`pipefail` kills the run silently on an empty match. Unattended extras: single `trap cleanup EXIT` (temp cleanup; on failure pull last 200 lines of `train.log` → `last-retrain.log` *before* teardown; pod delete; FCM push of outcome via `inference/notify.py:send_text` / `python -m inference.notify`, reusing the ROI-alert Firebase creds), `--terminate-after` server-side backstop, `flock` single-flight. Flags: `--no-split`, `--keep-pod`, `--no-swap`, `--auto-version` (`VERSION=auto-<date>` for cron — per-run weights = free rollback), `--pod-id=<id>` (reuse/resume on an existing pod, skip create). One-time setup: `RUNPOD_API_KEY` (env or `~/.runpod/config.toml`), `runpodctl ssh add-key` (private half `~/.ssh/runpod`), and for cron the NOPASSWD sudoers drop-in for `systemctl stop/start belfry-inference` (snippet in `runme.sh`). Deliberately **no model-quality gate** — home yard cam; a bad run self-corrects next run and `--auto-version` is the rollback. Real deploy command: `scripts/runpod-auto.sh --auto-version`.
...
The Caddyfile gates requests through three handlers, in order (see `cloud/Caddyfile`): (1) `@bearer` — requests with `Authorization: Bearer *` come from the mobile app; `forward_auth` to `/auth/verify` on the Orin validates the server JWT (200 → proxy on; 401 surfaced as-is, **not** redirected, so the app shows "session expired" instead of following a browser flow it can't render). (2) `@jwtcookie` — requests with a `belfry_jwt` cookie but no bearer header: this exists because **iOS `AVPlayer` won't attach the `Authorization` header to HLS segment / mp4 range sub-requests**, so the mobile app also stashes the JWT in a `belfry_jwt` cookie (NSURLSession sends cookies on every request); Caddy promotes the cookie to a `Bearer` header (`request_header`) and runs the same `/auth/verify` gate. Android's ExoPlayer honors per-request headers so it never needs this. (3) the catch-all `handle` — the cookie/oauth2-proxy browser path described above.
...
- Disk-aware retention runs as a FastAPI lifespan task (`dvr/retention.py`). Every `scan_interval_s` (default 60s) it `shutil.disk_usage`s the recordings volume; if usage > `evict_high_pct` (default 85), it deletes oldest mp4 files globally until usage < `evict_low_pct` (default 80). Files modified within the last 70 min are protected so MediaMTX never has its in-progress segment yanked. Per-camera dirs share the disk fairly under "oldest-globally" since all 8 cams record at similar bitrate.
- Each tick also sweeps the events DB: any row whose `ts_end` is older than the oldest surviving mp4 segment for the same camera is deleted along with its thumbnail JPEG (events outliving their footage are dead links). Runs every tick, not only above the high watermark.
...
### Retrain & deploy (one command)

```bash
cd ~/code/belfry
scripts/runpod-auto.sh --auto-version    # create pod → train → pull → delete pod → TRT-export → swap cameras.yaml → restart belfry-inference
```

No prompts, end-to-end (pushes an FCM notification on completion). `--auto-version` names the run `auto-<date>` so each run keeps its own `inference/belfry-auto-<date>.{pt,engine}` on disk — that's the rollback. Do **not** pass `--no-swap` for a real deploy; it stops after the weight pull (train-only). Common override: `BELFRY_GPU_ID="NVIDIA RTX PRO 6000 Blackwell Server Edition"` when you want a faster (pricier) run than the default `NVIDIA A40`. One-time prereqs are in the `scripts/runpod-auto.sh` Layout bullet (API key, `runpodctl ssh add-key`, NOPASSWD sudoers drop-in for `systemctl stop/start/restart belfry-inference` — installed via `runme.sh`).

**Rollback** a bad run: point `cameras.yaml`'s `yolo_pt:`/`yolo_engine:` lines at a prior `inference/belfry-*.{pt,engine}` pair and `sudo systemctl restart belfry-inference`.

- **Model**: the **belfry-v1 fine-tune** of YOLO11l — the stock COCO head surgically extended 80→86 (`scripts/extend-head.py`) with deer/coyote/raccoon/rabbit/squirrel/rat, trained `freeze=10` (backbone pinned) on our footage + Roboflow trailcam data. Exported to TensorRT FP16 once per Orin (engines are device-specific, at `inference/belfry-v1.engine`; weights `inference/belfry-v1.pt`) and held resident on the GPU. Active classes: `person, dog, cat, bird, car, truck` + `deer, coyote, raccoon, rabbit, squirrel, rat` — the rest of COCO (toaster, frisbee, etc.) is dropped at the `_YOLO_KEEP` filter in `inference/model.py`. car/truck detections are then merged into the single class **`vehicle`** by the Detector's `class_aliases` map (defaulted in `dvr/config.py`, overridable via `class_aliases:` in the `inference:` block — set `{}` to keep raw names). `event_classes` and `class_thresholds` key on the *raw* model names; only the emitted/stored class is renamed. The training set is untouched — label ids must stay COCO-aligned (car=2, truck=7) for the head-extension strategy, so the merge lives strictly downstream of the model; `training.STAGE_CLASS_FALLBACK` maps `vehicle` events back to `car` so 📥 stage-event and pre-seeding still work. The model path + the 12 `event_classes` are set in `cameras.yaml`'s `inference:` block (gitignored); revert to stock by removing the two path lines (defaults fall back to `inference/yolo11l.{pt,engine}`). Older `animal`/`vehicle` rows from the MegaDetector era still display correctly via the existing pip color map (old `car`/`truck` rows were migrated to `vehicle` 2026-06). See [fine-tune-plan.md](fine-tune-plan.md); base classes verified preserved post-swap (deer 0.95, cat 0.91, dog 0.62-0.69). coyote/squirrel under-fed (weak), rat is our-footage-only.
...
- **Event coalescing**: detections sampled at `record_fps` (default 1 fps). A detection above the per-class threshold opens a "run" for that (camera, class); subsequent detections within `cooldown_s` (default 10s) extend it; after `cooldown_s` of silence the run closes — one row written, peak-conf frame saved as a JPEG. A person walking past for 12 s is one row, not 12. The peak `bbox` is drawn in-place onto the frame (`draw_thumb_bbox`) before JPEG-encoding so `/events` grid thumbnails and the timeline-pip hover preview show exactly what was detected; the BGR colours mirror `overlay.js`'s `CLASS_COLOR` so the baked rectangles match the live overlay / pip palette.
...
- **Loopback RTSP + drain-thread reader**: the recorder reads `rtsp://127.0.0.1:8554/<cam>` (MediaMTX's loopback), so each camera only serves the one upstream consumer MediaMTX maintains — HLS, recording, and the inference recorder all multiplex off that one session. Inside the recorder a dedicated drain thread (see `recorder.py` above) keeps the socket consumed at ~30 fps so the H.264 stream stays clean even when inference takes a moment. Together these eliminated the per-cam "read failed; reopening" warnings that fired every 1–2 min and the resulting holes in events.db. Both pieces were needed: loopback alone didn't fix it because the bottleneck was on the consumer side, not the camera side.
...
- `/api/inference/live?cam=X` is an SSE pass-through proxy in the DVR (`dvr/server.py`) onto `127.0.0.1:9091/live?cam=X` inside the inference process. Keeps the OAuth gate in front; the inference port is loopback-only. `X-Accel-Buffering: no` so nginx flushes immediately; long timeouts since the SSE is long-lived (15 s server heartbeat keeps intermediaries from closing on idle).
...
- Browser side: `dvr/static/overlay.js` exposes `BoxOverlay` on `window`. Each instance owns a `<canvas>` layered over its host element (a `.video-wrap` per live tile, or `.playback-video-wrap` in playback live mode). The SSE connection is **lazy** — only opened when the "Show labels" pill is on. Without lazy SSE the four live-tile EventSource connections plus HLS pulls blew through the browser's HTTP/1.1 per-origin cap of 6 and broke video playback. The toggle calls `start()`/`stop()` on every registered overlay; CSS hides/shows the canvas independently so flipping is instant.
...
- Inference handler (`inference/playback.py`) pulls the same mp4 the DVR proxies for video — direct from MediaMTX `127.0.0.1:9996/get` — to a tmp file, then decodes in a worker thread with `cv2.VideoCapture` at 1 fps. Boxes per sampled frame are emitted as `data: {"ts": <unix>, "boxes": [...]}\n\n`. Worker stops promptly when the browser disconnects (a stop-flag the async generator sets in its `finally`).
- Inference goes flat-out — at ~30 ms/frame × 300 frames in a 5-minute window it finishes in ~9 s of GPU time, so by the time the user is mid-window every later box is already on the wire. Past-inference shares the live recorder's `Detector` lock, so live recording slows briefly while a past window processes but does not drop frames.
- **Stored-event fallback** (`_load_events_for_window`): re-inference samples at integer-second offsets from the window start, while the live recorder samples at arbitrary RTSP-arrival moments. A brief visit (a ~1 s bird flap) the live recorder caught can fall entirely between past-inference's sample points and never re-detect. The worker loads every event overlapping the window from `events.db` and, for each YOLO-sampled frame whose timestamp falls inside an event's `[ts_start, ts_end]`, appends the stored `peak_bbox` *only* when re-inference at that frame didn't already detect the same class. Net effect: a long person walk gets the moving re-detected box (no static duplicate), but a brief bird flash that re-detection misses still shows a static box for the event's duration. We keep re-inference (not pure stored events) because per-frame re-detection tracks moving subjects across the frame, whereas a stored event collapses to one peak position — worth the GPU cost for the moving-box UX.
...
- Keyboard: `j`/`k`/arrows navigate, `s` save, `p` save+promote, `x` trash, `h` hide/show boxes (view-only peek — clears the canvas to reveal the image underneath and pauses pointer editing so an invisible box can't be grabbed), `Esc` deselect.
...
- **Label-class filter** — a "Any label" dropdown above the category chips narrows the list to images whose `.txt` actually contains a box of that class, with per-class image counts. Keys on label *content*, not the staging folder: the folder is only a capture-time class hint and promoted images have no folder at all, so it's the only way to ask "show me every image with a raccoon box" across the whole set. Built for auditing a class after it over-fires in production. `/api/training/staging` returns a `classes: [ids]` list per item to back it (reads all ~4k label files, ~370 ms for the full listing — fine for an admin page). Counts are faceted by the state/category chips but not by the dropdown itself, and a selected class stays listed even at count 0 so the selection can't silently reset. Saving updates the item's classes in place but deliberately does *not* re-filter — an image you're mid-edit shouldn't vanish because you just deleted the box you were filtering on.
...
### Fine-tune (v1 shipped 2026-06-03)

belfry-v1 is trained and live — see [fine-tune-plan.md](fine-tune-plan.md) for the strategy/volume targets/head-extension/split/train/export-and-swap that was followed, plus the v1.1 notes. The pipeline that produced it: `import-external.py` (Roboflow trailcam data) → `split-dataset.py` → `extend-head.py` → `freeze=10` train on a rented GPU → TRT export on the Orin → swap via `cameras.yaml`'s `inference:` block + `_YOLO_KEEP`.
...
- **Phase 3 — Flutter mobile app (`mobile/`), iOS + Android.** Google native sign-in → `/auth/exchange` mints a 30-day server JWT (`dvr/auth.py`); app sends it via `Authorization: Bearer`. Live HLS grid, scrubback playback, cross-camera events, ROI-alert FCM push. **Android live; iOS brought up too** (needed Flutter ≥3.44 / Xcode 26 for iOS 26 — older Flutter segfaults at plugin registration). iOS HLS/mp4 auth rides the `belfry_jwt` cookie (AVPlayer drops the bearer header on sub-requests — see the Caddy `@jwtcookie` handler above; app side in `mobile/ios/Runner/AppDelegate.swift` + `mobile/lib/auth.dart`). iOS build/install notes in `mobile/ios-setup.md`. Build/run: `flutter build ios --release --dart-define-from-file=env.json` then `xcrun devicectl device install app …` (the `flutter run` launch step hangs on the dev Mac's devicectl).
...
- journald on the Orin is **persistent** (`/etc/systemd/journald.conf.d/10-belfry-persistent.conf`), so logs survive a reboot and an unclean restart leaves a post-mortem. It was runtime-only until 2026-08-09 — `journalctl --list-boots` reported a single synthetic boot spanning Feb 2025→now, which is why the Jul 28 and Aug 9 unclean reboots have no cause on record. `SystemMaxUse=1G` / `SystemKeepFree=10G` are pinned deliberately: `/` is the 3.6 TB recordings volume and already sits past the 85% eviction watermark, so an unbounded journal would compete with the space `dvr/retention.py` is working to reclaim. If you ever set this up on a fresh box: creating `/var/log/journal` and restarting journald is **not** sufficient — journald keeps writing to `/run` until it gets a flush signal, and `systemd-journal-flush.service` has already run for the current boot (as a no-op, before the directory existed) so it won't fire again. Send it by hand with `sudo journalctl --flush`, then confirm the log says `System Journal (/var/log/journal/…)` and not `Runtime Journal (/run/…)`.
