...
Moved to [fine-tune-plan.md](fine-tune-plan.md) — volume targets, surgical head extension strategy, train/val split, rent-a-GPU training flow, TRT export and swap.
...
- **Wildlife fine-tune (Phase B)** — YOLO11l alone has no `animal` class beyond the COCO `dog/cat/bird`, so anything else (deer, raccoon, coyote, fox) is invisible until we fine-tune. Data-collection tooling is shipped (training pipeline + labeler at `/training`); blocked on volume of labeled images. See [fine-tune-plan.md](fine-tune-plan.md) for the strategy/split/where-to-train/export-and-swap plan.
