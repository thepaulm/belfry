...
- `scripts/runpod-{1-send-to-pod,2-train-on-pod,3-receive-weights,4-export-and-swap}.sh` — the four-step RunPod retrain flow (bundle+send dataset via `runpodctl` p2p → train on the pod → pull weights back → TRT-export on the Orin + flip cameras.yaml + restart). `scripts/runpod-version` holds the `VERSION=` bumped once per run; step 1 bakes it into the train-script copy it ships so the pod side can't disagree on names. Step 2 never runs from the repo — it rides inside the tarball as `train-on-pod.sh`. Prereqs per run: bump the version, re-run `split-dataset.py`.
